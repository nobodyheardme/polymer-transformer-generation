...
def fraction_unique(gen, k=None, n_jobs=1, check_validity=True):
    ...
    return len(canonic) / len(gen)#前 k 个有效分子里有多少不重复
...
def fraction_valid(gen, n_jobs=1):
    ...
    Parameters:
        ...
        n_jobs: number of threads for calculation
    """
    gen = mapper(n_jobs)(get_mol, gen)#mapper在前面导入，n_jobs为进程，检测smiles的合法性
...
def novelty(gen, train, n_jobs=1):
    ...
    train_set = set(train)
    return len(gen_smiles_set - train_set) / len(gen_smiles_set)#有多少不在训练集里
...
def remove_invalid(gen, canonize=True, n_jobs=1):
    ...
    if not canonize:
        ...
        return [gen_ for gen_, mol in zip(gen, mols) if mol is not None]#去除get_mol中非法mol
    return [x for x in mapper(n_jobs)(canonic_smiles, gen) if
            x is not None]#去除canonic_smiles中非法smiles
...
def cos_similarity(ref_counts, gen_counts):
    ...
    keys = list(set(ref_counts.keys()) | set(gen_counts.keys()))

    ref_vec = np.array(
        [ref_counts.get(k, 0.0) for k in keys],
        dtype=np.float64
    )
    gen_vec = np.array(
        [gen_counts.get(k, 0.0) for k in keys],
        dtype=np.float64
    )
    ...
    sim = np.dot(ref_vec, gen_vec) / (ref_norm * gen_norm)
