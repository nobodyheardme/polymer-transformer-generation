...
_pains = pd.read_csv(PAINS_PATH, names=['smarts', 'names'])
...
def canonic_smiles(smiles_or_mol):
    ...
    if mol is None:
        return None
    return Chem.MolToSmiles(mol)#将字符串转化为标准模式
...
Tanimoto相似度：越像，Tanimoto 值就越大；越不像，就越小
    Parameters:
...
def mol_passes_filters(mol,
    ...
    * is not charged
    ...
    if ring_info.NumRings() != 0 and any(
            len(x) >= 8 for x in ring_info.AtomRings()#过滤8元以上的环
    ):
        return False
    ...
    if any(h_mol.HasSubstructMatch(smarts) for smarts in _filters):
        return False#使用作者自己整理的MCF和其他文献的PAINS 的 SMARTS 规则过滤
